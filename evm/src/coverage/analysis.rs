        ...
        match node.node_type {
            ...
            NodeType::DoWhileStatement | NodeType::WhileStatement => {
                let condition: Node = node
                    .attribute("condition")
                    .ok_or_else(|| eyre::eyre!("while statement has no condition"))?;
                ...
                let body =
                    node.body.ok_or_else(|| eyre::eyre!("while statement has no body node"))?;
            ...
            NodeType::BinaryOperation => {
                ...
                self.visit_expression(
                    node.attribute("leftExpression")
                        .ok_or_else(|| eyre::eyre!("binary operation has no left expression"))?,
                )?;
                self.visit_expression(
                    node.attribute("rightExpression")
                        .ok_or_else(|| eyre::eyre!("binary operation has no right expression"))?,
                )?;
                Ok(())
            }
